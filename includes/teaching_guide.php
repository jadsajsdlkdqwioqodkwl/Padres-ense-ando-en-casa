        ...
        <div style="max-height: 50vh; overflow-y: auto; padding-right: 10px;">
            <?php foreach($guide['steps'] as $step): ?>
                <div style="background: #f8f9fa; padding: 15px; margin-bottom: 12px; border-radius: 12px; border-left: 5px solid var(--accent); display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <button style="background: var(--accent); color: white; border: none; padding: 10px 15px; border-radius: 20px; cursor: pointer; font-weight: bold;" onclick="playTTS('<?php echo addslashes($step['en']); ?>')">🔊</button>
                </div>
            <?php endforeach; ?>
        </div>
        
        <button onclick="document.getElementById('parent-modal').style.display='none'" style="width: 100%; padding: 15px; background: var(--success); color: white; font-size: 18px; font-weight: bold; border: none; border-radius: 30px; margin-top: 20px; cursor: pointer;">¡Entendido, a jugar!</button>
    </div>
</div>
<?php endif; ?>